...
class BaseEmbedding(object):
	def __init__(self, model, device, ds_name_dataloaders, n_layers):
		self.device = device
	...
	def save_base_embeddings(self, model_name):
		...
		for ds_name, dls in self.ds_name_dataloaders.items():
			...
			for dl_name, dataloader in dls.items():
				...
				if not os.path.exists(path_embeddings): save_embeddings_npy = True
				if not os.path.exists(path_labels): save_labels_npy = True
