...
class GetEmbeddings():
	...
	def get_embeddings(self, ds_name, dataset):
		...
		for ds_type in ['train', 'test']:
			...
			if not os.path.exists(f'app/embeddings/{ds_name}/{ds_type}/labels.npy'):
				np.save(f'app/embeddings/{ds_name}/{ds_type}/labels.npy', np.array([-1 if x == 0 else x for x in dataset[ds_type]['label']]))
